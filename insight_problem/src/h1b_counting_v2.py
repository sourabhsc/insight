...
def sorting(property, dict_all, nrows, status_key):
    '''
    this functions is a counter for each unique state and occupation
    '''
...
if __name__ == '__main__':
    ...
    for j in range(2):
        ...
        with open('./output/top_10_%s.txt' % (output_stat['out_text'][j]), 'w') as f:
            f.write('%s;%s;%s\n' % (keys_extend[0], keys_extend[1], keys_extend[2]))
            for i in range(min(10,len(unique_key ))):
